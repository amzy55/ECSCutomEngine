...
public:
    ...
    void SetIndices(std::vector<unsigned short>& data);
...
protected:
    ...
    static std::vector<glm::vec4> ComputeTangents(
        const std::vector<glm::vec3>& positions, const std::vector<glm::vec3>& normals, const std::vector<glm::vec2>& uvs,
        const std::vector<uint32_t>& indices);
    unsigned int m_vao = 0;
    unsigned int m_ebo = 0;
    std::array<unsigned int, 9> m_vbo;
    uint32_t m_count = 0;
