...
layout(location = OCLUSION_SAMPLER_LOCATION)   uniform sampler2D s_occulsion;
...
{ 
    ...
    vec3 intensity = light.color_intensity.rgb * light.color_intensity.a; //vec3(1); //getLighIntensity(light, pointToLight);
...
{       
    ...
    // if(albedo.a < 0.2f) // TODO: Bring this from material
    //    discard;
    ...
    vec3 V = normalize(bee_eyePos - v_position);    // View
    vec3 N = normal;                                // Normal
    ...
    {        
        ...
        frag_color = vec4(light.direction, 1);
